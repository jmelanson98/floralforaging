...
#plot comparisons
result = read.csv("analysis/floral_foraging/saved.csv")
result$rho1 = as.numeric(result$rho1)
result$rho1group = as.factor(rho1)
result$rho2group = as.factor(rho2)
result$rho3group = as.factor(rho3)


pd = position_dodge(width = 0.03)

result1 = result[result$rho3 ==0,]

p1 = ggplot(result1, aes(x = rho1, y = rho1_50, color = rho2, group = rho1group)) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "grey40") +
  geom_point(aes(group = interaction(rho1group, rho2)), position = pd, size = 1) +
  geom_errorbar(aes(ymin = rho1_2.5, ymax = rho1_97.5, group = interaction(rho1group, rho2)),
                position = pd,
                width = 0) +
  scale_color_continuous(
    name = expression(rho[2])) +
  xlab(expression(rho["1G"])) +
  ylab(expression(rho["1I"])) +
  theme_bw()

pd = position_dodge(width = 0.05)
p2 = ggplot(result1, aes(x = rho2, y = rho2_50, color = rho1, group = rho2group)) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "grey40") +
  geom_point(aes(group = interaction(rho2group, rho1)), position = pd, size = 1) +
  geom_errorbar(aes(ymin = rho2_2.5, ymax = rho2_97.5, group = interaction(rho2group, rho1)),
                position = pd,
                width = 0) +
  ylim(c(-1,1)) +
  scale_color_continuous(
    name = expression(rho[1])) +
  xlab(expression(rho["2G"])) +
  ylab(expression(rho["2I"])) +
  theme_bw()

result2 = result[result$rho2 ==0,]
pd = position_dodge(width = 0.01)
p3 = ggplot(result2, aes(x = rho3, y = rho3_50, color = rho1, group = rho3group)) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "grey40") +
  geom_point(aes(group = interaction(rho3group, rho1)), position = pd, size = 1) +
  geom_errorbar(aes(ymin = rho3_2.5, ymax = rho3_97.5, group = interaction(rho3group, rho1)),
                position = pd,
                width = 0) +
  scale_color_continuous(
    name = expression(rho[1])) +
  xlab(expression(rho["3G"])) +
  ylab(expression(rho["3I"])) +
  theme_bw()

grid = (p1 / p2 / p3) + 
  plot_annotation(tag_levels = "A")
ggsave("analysis/kernel_locations/modeltesting/report_figures/interactions.png", grid,
